...
#     browser.get(url)
#     divs = browser.find_elements(By.CLASS_NAME, 'check')
#     for div in divs:
#         value = div.get_attribute('value')
...
with webdriver.Chrome() as browser:
    browser.get(url)
    SECRET_SUMM = str(((12434107696 * 3) * 2) + 1)
    elements = browser.find_elements(By.TAG_NAME, 'option')
    for element in elements:
        if element.text == SECRET_SUMM:
            element.click()
            break
    browser.find_element(By.CLASS_NAME, 'btn').click()
    print(browser.find_element(By.ID, 'result').text)
